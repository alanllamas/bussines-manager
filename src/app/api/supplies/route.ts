// Proxy route: /api/supplies
// Forwards requests to the Strapi `supplies` collection (Insumos).
...
// filtering, pagination, and populate fields.
// cache: 'no-store' prevents Next.js from caching — supply list must always be fresh.
...
// POST /api/supplies
// Creates a new supply (Insumo). populate=supply_variants returns the variants linked to this
// supply in the response so the UI can display them immediately without a separate GET.
